...
from functions import clip_black_norm

OUT_DIR = os.path.join(SCRIPT_DIR, "ch_clipblack_output")
...
for i in range(1, 22):
    ...
    if classify(gray) != "type_a":
        continue
    type_a_ids.append(i)
    result = clip_black_norm(gray, A_THRESHOLD)
    results[i] = result
    _imwrite(os.path.join(OUT_DIR, f"BC{i}_clipblack.jpg"), result)
    print(f"BC{i} saved")
...
for j in range(len(type_a_ids), len(axes)):
    axes[j].axis('off')
plt.suptitle("TypeA Checkpoint: clip_black_norm (threshold=110)", fontsize=13)
plt.tight_layout()
