...
from functions import fill_holes, region_filter
...
OUT_DIR = os.path.join(SCRIPT_DIR, "ch_regionfilter_output")
...
for i in range(1, 22):
    ...
    if not is_type_b(otsu):
        continue
    type_b_ids.append(i)
    filled = fill_holes(otsu)
    selected = region_filter(filled)
    results[i] = selected
    _imwrite(os.path.join(OUT_DIR, f"BC{i}_regionfilter.jpg"), selected)
    print(f"BC{i} saved")
...
for j in range(len(type_b_ids), len(axes)):
    axes[j].axis('off')
plt.suptitle("TypeB Checkpoint: region_filter  (최대 컴포넌트)", fontsize=13)
plt.tight_layout()
