...
from functions import fill_holes
...
OUT_DIR = os.path.join(SCRIPT_DIR, "ch_fillholes_output")
...
for i in range(1, 22):
    ...
    results[i] = filled
    _imwrite(os.path.join(OUT_DIR, f"BC{i}_fillholes.jpg"), filled)
    print(f"BC{i} saved")
...
for j in range(len(type_b_ids), len(axes)):
    axes[j].axis('off')
plt.suptitle("TypeB Checkpoint: fill_holes", fontsize=13)
plt.tight_layout()
